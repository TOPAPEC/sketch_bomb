...
CLASSES = ["cat", "car", "tree", "house", "flower"]
...
class FluxSketch2Image:
    ...
    def quickdraw_to_image(self, drawing_data):
        ...
        for stroke in strokes:
            ...
            if len(points) > 1:
                draw.line(points, fill='black', width=max(3, self.img_size // 170))
        return image
    ...
    def create_paired_grid(self, sketches: list[Image.Image], results: list[Image.Image], 
        ...
        grid = Image.new('RGB', (img_size * cols + label_width, img_size * num_classes), 'white')
        ...
        for class_idx, label in enumerate(labels):
            for i in range(samples_per_class):
                ...
                grid.paste(sketches[idx], (i * 2 * img_size, class_idx * img_size))
                grid.paste(results[idx], (i * 2 * img_size + img_size, class_idx * img_size))
            
            label_x = cols * img_size + 10
            label_y = class_idx * img_size + img_size // 2 - 30
            draw.text((label_x, label_y), f"{label}", fill='black', font=font)
            draw.text((label_x, label_y + 35), f"score: {scores[class_idx]:.3f}", fill='gray', font=font)
        
        return grid
    
    def batch_colorize(self, control_images, labels, num_inference_steps=28, controlnet_conditioning_scale=0.6):
        results = []
        for i, (ctrl_img, label) in enumerate(zip(control_images, labels)):
            print(f"  {i+1}/{len(control_images)} ({label})")
            canny_img = self.preprocess_canny(ctrl_img)
            result = self.pipe(
...
def main():
    samples_per_class = 2
    out_dir = Path("out3/fluxv1")
    out_dir.mkdir(parents=True, exist_ok=True)
    
    pipeline = FluxSketch2Image(device="cuda" if torch.cuda.is_available() else "cpu", img_size=1024)
    ...
    print("\nGenerating colored images with FLUX...")
    colored = pipeline.batch_colorize(quickdraw_imgs, all_labels)
    ...
    grid.save(out_dir / "paired_results.png")
    
    print(f"\nSaved to {out_dir}/paired_results.png")
