...
class Sketch2Image:
    ...
    def setup_models(self, sdxl_model: str):
        print(f"Loading models with {sdxl_model}...")
        if self.controlnet is None:
            self.controlnet = ControlNetUnionModel.from_pretrained(
                "xinsir/controlnet-union-sdxl-1.0", torch_dtype=torch.float16).to(self.device)
        self.pipe = StableDiffusionXLControlNetUnionPipeline.from_pretrained(
            sdxl_model, controlnet=self.controlnet, torch_dtype=torch.float16).to(self.device)
    ...
    def batch_colorize(self, control_images, labels, num_inference_steps=40, strong_steps=15):
        ...
        for i, (ctrl_img, label) in enumerate(zip(control_images, labels)):
            ...
            result = self.pipe(
                ...
                controlnet_conditioning_scale=0.6, control_guidance_start=0.0, control_guidance_end=control_end,
...
def main():
    samples_per_class = 4
    out_dir = Path("out3/comparingsdxlDumps")
    out_dir.mkdir(parents=True, exist_ok=True)
    
    pipeline = Sketch2Image(device="cuda" if torch.cuda.is_available() else "cpu", img_size=1024)
    ...
    for sdxl_model in SDXL_DUMPS:
        model_name = sdxl_model.replace("/", "_")
        print(f"\n{'='*50}\nProcessing: {sdxl_model}\n{'='*50}")
        
        pipeline.setup_models(sdxl_model)
